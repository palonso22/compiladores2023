...
search (V _ (Global n)) e k = do
                                ...
                                case mDecl of
                                    ...
                                    Just t -> destroy t k
search (Let i n ty t1 t2) e k = search t1 e (FrLet e (fromScope t2):k)


search (V _ (Bound n)) e k = destroy (e!!n) k
search q@(Const _ (CNat c)) e k = destroy (N c) k
search (Lam (_, fty)  n ty t) e k = destroy (Clos (ClosFun fty e n ty (fromScope t)))  k
search (Fix (_, fty)  f tf x tv t) e k = destroy (Clos (ClosFix fty e f tf x tv (fromScope2 t))) k
...
destroy v (FrClos (ClosFun _ e _ _ t):k) = search t (v:e) k

destroy v ((FrClos q@(ClosFix _ e _ _ _ _ t)):k) = search t (v:Clos q:e) k
